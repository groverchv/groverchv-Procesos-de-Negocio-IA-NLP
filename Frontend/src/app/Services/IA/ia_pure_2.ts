...
interface NodeData {id?:string; type?:string; label?:string; x?:number; y?:number; width?:number; height?:number; fontSize?:number;} interface EdgeData {}
...
→ { "user_feedback": "¡Perfecto! Puse la decisión ¿Aprobado? con sus dos caminos.", "commands": [
  ...
  private parseLaneReference(message: string): string | null {
    ...
    const byNumber = message.match(/(?:en|a|al|hacia)\s+(?:el\s+|la\s+)?(?:calle|carril|swimlane|zona|area|seccion)\s*(\d+)/i);
    if (byNumber?.[1]) return byNumber[1];

    const byName = message.match(/(?:en|a|al|hacia)\s+(?:el\s+|la\s+)?(?:calle|carril|swimlane|zona|area|seccion)\s+([\p{L}\d_-]+)/iu);
  ...
  private localFallback(userMessage: string, currentNodes: NodeData[]): IaResponse {
    ...
    for (const step of steps) {
      ...
      const addNode = this.tryParseAddNode(step, simulatedNodes);
      if (addNode) {
  ...
  private tryParseAddNode(step: string, currentNodes: NodeData[]): DiagramCommand | null {
    ...
    const asksActivity = /(actividad|tarea|task|cuadrit|cajit|bloque|paso)/.test(this.normalizeForSearch(step));
    if (asksActivity) {
    ...
    if (nodeType === 'swimlane') {
      ...
      const alreadyExists = currentNodes
        .filter(n => n.type === 'swimlane' && !!n.label)
        .some(n => this.normalizeForSearch(n.label || '') === this.normalizeForSearch(label));
      
      if (alreadyExists) {
        label = `${label} ${currentNodes.filter(n => n.type === 'swimlane').length + 1}`;
      }
...
const result = service.localFallback('Agrega una calle venta. escribi esa intruccion y no funciono', [{id: 'n1', type: 'swimlane', label: 'Calle 1', width: 300, x: 0, y: 0}]);
console.log(JSON.stringify(result, null, 2));

const service = new IaService();
const result = service.localFallback('crea una calle venta,', [{id: 'n1', type: 'swimlane', label: 'Calle 1', width: 300, x: 0, y: 0}]);
console.log(JSON.stringify(result, null, 2));
