...
var IaService = /** @class */ (function () {
    ...
    IaService.prototype.parseLaneReference = function (message) {
        if (!message)
            return null;
        var byNumber = message.match(/\b(?:en|a|al|hacia)\b\s+(?:el\s+|la\s+)?(?:calle|carril|swimlane|zona|area|seccion|pista|pool|departamento|sector|pasillo)\s*(\d+)/i);
        if (byNumber === null || byNumber === void 0 ? void 0 : byNumber[1])
            return byNumber[1];
        var byName = message.match(/\b(?:en|a|al|hacia|sobre|dentro)\b\s+(?:el\s+|la\s+)?(?:calle|carril|swimlane|zona|area|seccion|pista|pool|departamento|sector|pasillo)\s+([\p{L}\d_-]+)/iu);
    ...
    IaService.prototype.localFallback = function (userMessage, currentNodes) {
        ...
        for (var _i = 0, steps_1 = steps; _i < steps_1.length; _i++) {
            ...
            var addNodes = this.tryParseAddNode(step, simulatedNodes);
            if (addNodes) {
                var arr = Array.isArray(addNodes) ? addNodes : [addNodes];
                for (var _a = 0, arr_1 = arr; _a < arr_1.length; _a++) {
                    var addNode = arr_1[_a];
    ...
    IaService.prototype.tryParseAddNode = function (step, currentNodes) {
        ...
        var count = 1;
        var countMatch = s.match(/(?:agrega|crea|añade|inserta|pon|ponme|dame|haz)\s+(un|una|dos|tres|cuatro|cinco|\d+)\b/i);
        if (countMatch && countMatch[1]) {
            ...
            if (p === 'un' || p === 'una')
                ...
                count = parseInt(p, 10) || 1;
        }
        // Caso prioritario: "agrega actividad/tarea en la calle/carril X"
        // No debe crear una calle nueva si la calle ya existe.
        var asksActivity = /(actividad|tarea|task|cuadrit|cajit|bloque|paso)\b/i.test(this.normalizeForSearch(step));
        if (asksActivity) {
        ...
        if (nodeType === 'swimlane') {
            ...
            var cmds_2 = [];
            var virtualNodes = __spreadArray([], currentNodes, true);
            var _loop_1 = function (i) {
                var currentLabel = label_1;
                if (count > 1 && label_1 !== this_1.getNextLaneName(virtualNodes)) {
                    currentLabel = "".concat(label_1, " ").concat(i + 1);
                }
                var alreadyExists = virtualNodes
                    .filter(function (n) { return n.type === 'swimlane' && !!n.label; })
                    .some(function (n) { return _this.normalizeForSearch(n.label || '') === _this.normalizeForSearch(currentLabel); });
                if (alreadyExists) {
                    currentLabel = "".concat(currentLabel, " ").concat(virtualNodes.filter(function (n) { return n.type === 'swimlane'; }).length + 1);
                }
                var nextX = this_1.getNextLaneX(virtualNodes);
                cmds_2.push({
                    action: 'add_node',
                    nodeType: 'swimlane',
                    label: this_1.capitalize(currentLabel),
                    x: nextX,
                    y: 0,
                    width: 300,
                    height: 520
                });
                virtualNodes.push({ type: 'swimlane', label: currentLabel, x: nextX, width: 300 });
                // Detección de subcomandos en la misma frase (Ej. "con una actividad proceso")
                var innerMatch = step.match(/(?:con|que\s+tenga|incluyendo)\s+(?:un|una|dos|tres)?\s*(actividad|tarea|proceso)(?:\s+(?:llamada|llamado|nombre|con\s+nombre)?\s*([\p{L}\d_ -]+))?/iu);
                if (innerMatch) {
                    var innerLabel = innerMatch[2] ? innerMatch[2].trim() : 'Actividad';
                    innerLabel = innerLabel.replace(/^(una|un|el|la)\s+/i, '').trim();
                    cmds_2.push({
                        action: 'add_node',
                        nodeType: 'activity',
                        label: this_1.capitalize(innerLabel),
                        x: nextX + 50,
                        y: 70
                    });
                }
...
var result = service.localFallback('crea una calle venta', [{ id: 'n1', type: 'swimlane', label: 'Calle 1', width: 300, x: 0, y: 0 }]);
console.log(JSON.stringify(result, null, 2));
var result2 = service.localFallback('agrega una calle venta con una actividad proceso', [{ id: 'n1', type: 'swimlane', label: 'Calle 1', width: 300, x: 0, y: 0 }]);
