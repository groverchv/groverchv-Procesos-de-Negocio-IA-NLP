...
→ { "user_feedback": "¡Perfecto! Puse la decisión ¿Aprobado? con sus dos caminos.", "commands": [
  ...
  private tryParseAddNode(step: string, currentNodes: NodeData[]): DiagramCommand | DiagramCommand[] | null {
    ...
    const asksActivity = /(actividad|tarea|task|cuadrit|cajit|bloque|paso)\b/i.test(this.normalizeForSearch(step));
    if (asksActivity) {
...
const result2 = service.localFallback('agrega una calle venta con una actividad proceso', [{id: 'n1', type: 'swimlane', label: 'Calle 1', width: 300, x: 0, y: 0}]);
